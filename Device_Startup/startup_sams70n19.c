 ...
 * \file
 *
 * \brief GCC startup file for ATSAMS70N19
 *
...
/* Initialize segments */
extern uint32_t _s_itcm_from;
extern uint32_t _s_itcm;
extern uint32_t _e_itcm;
 ...
 */
// Not sure why this has to have optimization turned off, but there we are.
__attribute__((optimize("O0"))) void Reset_Handler(void)
{
	...
	__ISB();
        ...
        uint32_t *pSrc, *pDest, *pDestEnd;

		/* Copy the vector table to ITCM */
		...
		pSrc = (uint32_t*)&_s_itcm_from;
		pDest = (uint32_t*)&_s_itcm;
		pDestEnd = (uint32_t*)&_e_itcm;
		for (; pDest < pDestEnd;) {
			*pDest++ = *pSrc++;
		}
